...
class MorphingAscii {
  ...
  morphCharacters(fromData, toData, progress) {
    ...
    for (let i = 0; i < fromData.data.length; i++) {
      ...
      // In minimal mode, make image ASCII invisible (white on white)
      if (isMinimalMode) {
        result += `<span style="color:rgb(255,255,255)">${morphed.char}</span>`;
      } else {
...
function morphTextOverlay(progress) {
  ...
  const textColor = isMinimalMode
    ? { r: 0, g: 0, b: 0 }
    : brightenColor(DUOTONE.light, 0.5);
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (fromCell.isText || toCell.isText) {
        ...
        // Use the brightened light color from duotone
        result += `<span style="color:rgb(${textColor.r},${textColor.g},${textColor.b})">${char}</span>`;
      } else {
        // Empty space
        result += ' ';
      }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    await document.fonts.load('203px "Druk Wide"');
