    ...
    for (let y = 0; y < height; y += sampleRate) {
      ...
      for (let x = 0; x < width; x += sampleRate) {
        ...
        if (isText) {
          ...
          // Use black and white only (random grayscale value heavily biased towards white for visibility)
          const grayValue = Math.floor(Math.pow(Math.random(), 0.25) * 256);

          // Assign random swap timing for morphing
          const swapTiming = Math.random();

          row.push({ char, grayValue, isText: true, swapTiming });
        } else {
          // Empty space
          row.push({ char: ' ', grayValue: 0, isText: false, swapTiming: 0 });
        }
      }
      asciiData.push(row);
    }
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (fromCell.isText || toCell.isText) {
        ...
        if (progress < swapThreshold) {
          ...
          char = fromCell.char;
          grayValue = fromCell.grayValue;
        } else {
          // Swapped to target character
          char = toCell.char;
          grayValue = toCell.grayValue;
        }
