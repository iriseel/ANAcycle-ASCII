...
class MorphingAscii {
  ...
  handleScroll() {
    ...
    const targetIndex = Math.min(sectionIndex, this.asciiDataCache.length - 1);

    // Trigger transition if we've scrolled to a new section
    // This will interrupt any ongoing transition
    if (targetIndex !== this.targetImageIndex) {
